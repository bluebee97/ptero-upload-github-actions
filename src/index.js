    ...
    for (const serverId of serverIds) {
      ...
      for (const source of fileSourcePaths) {
        ...
        if (decompressTarget && isArchiveFile(targetFile)) {
          await decompressFile(serverId, path.basename(source));
          await deleteFile(serverId, source);
        }
      }
...
function isArchiveFile(fileName) {
  ...
  return [".zip", ".tar", ".tar.gz", ".gz", ".tgz", ".rar"].includes(ext);
}
